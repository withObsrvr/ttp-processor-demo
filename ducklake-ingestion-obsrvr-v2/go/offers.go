...
// Captures DEX orderbook state with selling/buying assets, amount, and price
// Obsrvr playbook: core.offers_snapshot_v1
func (ing *Ingester) extractOffers(lcm *xdr.LedgerCloseMeta) []OfferData {
	...
	for {
		...
		// Extract offer data
		offerData := extractOfferData(offerEntry, ledgerSeq, closedAt)
...
func extractOfferData(entry *xdr.OfferEntry, ledgerSeq uint32, closedAt time.Time) OfferData {
	// Identity
...
func parseAsset(asset xdr.Asset) (assetType string, code *string, issuer *string) {
	...
	default:
		...
		return unknownType, &unknownCode, &unknownIssuer
	}
}
