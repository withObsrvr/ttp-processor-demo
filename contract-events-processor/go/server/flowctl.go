...
type FlowctlController struct {
	...
	eventServer       *ContractEventServer
	endpoint          string
}

// NewFlowctlController creates a new controller for flowctl integration
func NewFlowctlController(eventServer *ContractEventServer) *FlowctlController {
	// Default values
...
func (fc *FlowctlController) RegisterWithFlowctl() error {
	...
	serviceInfo := &flowctlpb.ServiceInfo{
		...
		OutputEventTypes: []string{"contract_event_service.ContractEvent"},
		HealthEndpoint:   healthEndpoint,
		MaxInflight:      100,
		Metadata: map[string]string{
			"network":        network,
			"processor_type": "contract_events",
		},
	}
	...
	if err != nil {
		...
		fc.serviceID = "sim-contract-events-processor-" + time.Now().Format("20060102150405")
...
func (fc *FlowctlController) sendHeartbeat() {
	...
	heartbeat := &flowctlpb.ServiceHeartbeat{
		...
		Metrics: map[string]float64{
			"ledgers_processed":      float64(metrics.LedgersProcessed),
			"events_sent":            float64(metrics.EventsSent),
			"events_filtered":        float64(metrics.EventsFiltered),
			"total_events_found":     float64(metrics.TotalEventsFound),
