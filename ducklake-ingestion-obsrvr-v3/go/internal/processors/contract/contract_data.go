...
import (
	...
	"github.com/withObsrvr/ttp-processor-demo/ducklake-ingestion-obsrvr-v3/go/internal/processors/utils"
)
...
// SerializeScVal serializes an XDR ScVal into string representations
// Returns two maps: one with XDR base64 encoding, one with decoded values
func SerializeScVal(val xdr.ScVal) (map[string]string, map[string]string) {
	output := make(map[string]string)
	outputDecoded := make(map[string]string)

	// Serialize to base64 XDR
	outputXDR, err := xdr.MarshalBase64(val)
	if err == nil {
		output["xdr"] = outputXDR
	}

	// Add type information
	output["type"] = val.Type.String()
	outputDecoded["type"] = val.Type.String()

	return output, outputDecoded
}
