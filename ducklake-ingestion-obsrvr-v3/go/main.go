...
const (
	...
	ProcessorVersion = "3.0.0" // v3: DuckDB catalog default + critical bug fixes
...
type Config struct {
	...
	DuckLake struct {
		...
		PartitionSize         int    `yaml:"partition_size"` // Ledgers per partition (default: 64000, matching Galexie)
		...
		DataInliningRowLimit  int    `yaml:"data_inlining_row_limit"`  // Rows to buffer before Parquet flush (0 = disabled)
...
func (ing *Ingester) initializeDuckLake() error {
	...
	if ing.config.DuckLake.DataPath != "" {
		// Remote DuckLake with DATA_PATH and optional METADATA_SCHEMA and DATA_INLINING_ROW_LIMIT
		if ing.config.DuckLake.MetadataSchema != "" {
			if ing.config.DuckLake.DataInliningRowLimit > 0 {
				// With data inlining (DuckDB catalog only)
				attachSQL = fmt.Sprintf(
					"ATTACH '%s' AS %s (DATA_PATH '%s', METADATA_SCHEMA '%s', DATA_INLINING_ROW_LIMIT %d)",
					...
					ing.config.DuckLake.DataInliningRowLimit,
				)
				log.Printf("Data inlining enabled: buffering %d rows before Parquet flush", ing.config.DuckLake.DataInliningRowLimit)
			} else {
				// No data inlining
				attachSQL = fmt.Sprintf(
			...
			if ing.config.DuckLake.DataInliningRowLimit > 0 {
				attachSQL = fmt.Sprintf(
					"ATTACH '%s' AS %s (DATA_PATH '%s', DATA_INLINING_ROW_LIMIT %d)",
					ing.config.DuckLake.CatalogPath,
					ing.config.DuckLake.CatalogName,
					ing.config.DuckLake.DataPath,
					ing.config.DuckLake.DataInliningRowLimit,
		...
		// Local DuckDB file (no DATA_PATH, so no data inlining support)
		attachSQL = fmt.Sprintf(
			"ATTACH '%s' AS %s",
			ing.config.DuckLake.CatalogPath,
			ing.config.DuckLake.CatalogName,
	...
	// Setup partitioning for optimal query performance
	// This configures tables to partition every 100K ledgers
	// Note: Only affects NEW data, existing data keeps original layout
	if err := ing.setupPartitioning(); err != nil {
		log.Printf("Warning: Partitioning setup failed (continuing anyway): %v", err)
	}
...
func (ing *Ingester) Start(ctx context.Context) error {
	...
	for {
		...
		case result, ok := <-ledgerChan:
			...
			// When end_ledger is set to a specific value, ALWAYS stop there
