...
func main() {
	log.Println("Starting flowctl integration example for TTP Processor")

	// Prepare registration info
	serviceID := "ttp-processor-" + time.Now().Format("20060102150405")
	
	registration := ServiceRegistration{
		ServiceType:      "PROCESSOR",
		InputEventTypes:  []string{"raw_ledger_service.RawLedgerChunk"},
		OutputEventTypes: []string{"event_service.TokenTransferEvent"},
		HealthEndpoint:   "http://localhost:8088/health",
		MaxInflight:      100,
		Metadata: map[string]string{
			"network":        "testnet",
			"processor_type": "token_transfer",
		},
	}
			...
			case <-ticker.C:
				heartbeat := ServiceHeartbeat{
					...
					Metrics: map[string]float64{
						"success_count":          float64(85),
						"error_count":            float64(3),
						"total_processed":        float64(88),
						"total_events_emitted":   float64(140),
						"last_processed_ledger":  float64(45678923),
					},
				}
