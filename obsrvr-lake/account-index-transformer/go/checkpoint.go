...
func (cm *CheckpointManager) ensureTable(ctx context.Context) error {
	schema := "index"
	if parts := strings.Split(cm.config.Table, "."); len(parts) == 2 {
		schema = parts[0]
	}
	if _, err := cm.db.ExecContext(ctx, fmt.Sprintf("CREATE SCHEMA IF NOT EXISTS %s", schema)); err != nil {
	...
	_, err := cm.db.ExecContext(ctx, fmt.Sprintf(`
		INSERT INTO %s (id, last_ledger_sequence)
		VALUES (1, 0)
		ON CONFLICT (id) DO NOTHING
	`, cm.config.Table))
	if err != nil {
		return fmt.Errorf("failed to seed checkpoint row: %w", err)
	}
	return nil
}
