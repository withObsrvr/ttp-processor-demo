...
func (w *Writer) extractEffects(rawLedger *pb.RawLedger) ([]EffectData, error) {
	...
	switch lcm.V {
	case 0:
		h := lcm.MustV0().LedgerHeader
		closedAt = time.Unix(int64(h.Header.ScpValue.CloseTime), 0).UTC()
		ledgerSeq = uint32(h.Header.LedgerSeq)
	case 1:
		h := lcm.MustV1().LedgerHeader
		closedAt = time.Unix(int64(h.Header.ScpValue.CloseTime), 0).UTC()
		ledgerSeq = uint32(h.Header.LedgerSeq)
	case 2:
		h := lcm.MustV2().LedgerHeader
		closedAt = time.Unix(int64(h.Header.ScpValue.CloseTime), 0).UTC()
		ledgerSeq = uint32(h.Header.LedgerSeq)
	}
	...
	for {
		...
		for opi, op := range tx.Envelope.Operations() {
			ow := &opWrapper{
				...
				op:        op,
				ledgerSeq: ledgerSeq,
				network:   w.config.Source.NetworkPassphrase,
				closedAt:  closedAt,
			}
