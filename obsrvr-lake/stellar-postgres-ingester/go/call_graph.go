...
func extractCallsFromAuthInvocation(
	...
	// Some auth payloads carry a CONTRACT_FN arm whose ContractAddress is not a
	// contract ScAddress (e.g. an account/muxed arm). The generated union getter
	// returns ok=false for non-contract arms, so we skip those cleanly instead of
	// dereferencing ContractAddress.ContractId directly and panicking the live
	// ingester.
...
func scAddressContractStrKey(address xdr.ScAddress) (string, bool) {
	...
	if !ok {
		...
		// per-occurrence; the aggregate is observable without log spam.
...
// to detect silent call-graph under-counting.
