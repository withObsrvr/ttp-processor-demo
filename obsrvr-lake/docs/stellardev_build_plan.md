...
5. Put these fixtures in docs or test config so all tickets can reuse them. **→ Recorded in [`stellar dev_fixtures.md`](./stellar dev_fixtures.md).** sorobandomains contracts (mainnet) for Ticket 2 are already captured there; addresses for Tickets 3–5 are still TODO.
...
- **Ship the fix now** — the bug is code-verified without a fixture; validate on **testnet** against a real contract exercising create/update/delete (no mocks). Delete fixture located: contract `CCJQB4EE…`, key `eb4e8c42…` create@3207019 → delete@3207025 — full details in [`stellar dev_fixtures.md`](./stellar dev_fixtures.md).
