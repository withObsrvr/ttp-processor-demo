...
// flushInlinedBronzeTablesInternal flushes inlined rows from the DuckLake catalog
// into Parquet files so they can participate in subsequent merge compaction.
func (c *DuckDBClient) flushInlinedBronzeTablesInternal(ctx context.Context, tables []string) error {
	log.Println("🧊 Flushing inlined DuckLake data for Bronze tables before merge...")
	...
	if successCount == 0 {
		return fmt.Errorf("failed to flush inlined data for all %d Bronze tables", len(tables))
	}
	return nil
}
...
func (c *DuckDBClient) RunCheckpoint(ctx context.Context, maxCompactedFiles int) error {
	...
	if err := c.flushInlinedBronzeTablesInternal(ctx, HighVolumeBronzeTables); err != nil {
		log.Printf("⚠️  Bronze inlined-data flush completed with errors: %v", err)
	}
